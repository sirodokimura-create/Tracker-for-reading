...
class FuzzySearchEngine:
    ...
    def extract_trigrams(self, word: str) -> Set[str]:
        """
        Разбивает слово на трехбуквия (триграммы)
        Пример: "книга" -> {"кни", "ниг", "ига"}
        """
    ...
    def calculate_trigram_similarity(self, query: str, text: str) -> float:
        """
        Вычисляет степень схожести двух строк на основе триграмм
        ...
        # Получаем триграммы для каждого слова текста
        text_trigrams = set()
        ...
        # Вычисляем пересечение триграмм
    ...
    def search_books(self, query: str, threshold: float = 0.3) -> List[Dict]:
        """
        Поиск книг на сайте с учетом нечеткого совпадения
        query: поисковый запрос
        threshold: порог схожести (0-1), ниже которого результаты отбрасываются
        """
    ...
    def _parse_search_results(self, soup: BeautifulSoup, query: str, threshold: float) -> List[Dict]:
        """
        Парсит страницу результатов поиска и вычисляет релевантность
        """
        books = []
        ...
        for card in book_cards:
            ...
            if book_data:
                # Вычисляем релевантность для названия и автора
